...
def forecast_energy(solar: dict, wind: dict, recommended_technology: str, land_area_hectares: float = 5.0) -> dict:
    if recommended_technology == "solar":
        ...
        capex = capacity_mw * settings.SOLAR_CAPEX_PER_MW_USD
        ...
        capex = capacity_mw * settings.WIND_CAPEX_PER_MW_USD
    else:  # hybrid
        ...
        capex = (round(land_area_hectares / 2.0, 3) * settings.SOLAR_CAPEX_PER_MW_USD) + \
                (round((land_area_hectares / 8.0) * 2.5, 3) * settings.WIND_CAPEX_PER_MW_USD)
    ...
    annual_revenue = round(year1 * settings.PRICE_PER_MWH_USD, 2)
